...
> **保守的な推定**とは、実際の最悪応答時間を下回らないように、安全側に大きく見積もることです。そのため、$\tilde{R}_i \le D_i$ なら、タスクがデッドラインを守ることを保証できます。
...
$V_i^0$の下付きの$i$はタスク、上付きの0は時刻を表します。

また、$w_0$の0は時刻やタスク番号ではなく、nice値が0のときの基準を表します。
...
$L$は、CFSのtarget latency（ターゲット・レイテンシ）です。
...
> 干渉タスク$\tau_j$が対象タスク$\tau_i$より有利になれるvruntime差は、最大でも$\alpha_i+L/2$
