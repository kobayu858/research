...
| スキーム | 重み $W_i$ | 特徴 |
|---------|-----------|------|
| **Equal（等重み）** | $W_i = L_i$ | 全chainを同等に扱う |
| **Linear（線形）** | $W_i = L_i / Pr(i)$ | 優先度に比例して重み付け |
| **Logarithmic（対数）** | $W_i = L_i / 10^{3 \cdot Pr(i)}$ | 高優先度のchainを極端に重視 |
...
**対数重み付けが悪化した原因:** エグゼキュータ7にほぼ全ての知覚ノードが集中し、複数chainのレイテンシが大幅に増加。
...
- **対数重み付け**は特定chainを過度に優先し、他のchainが悪化
